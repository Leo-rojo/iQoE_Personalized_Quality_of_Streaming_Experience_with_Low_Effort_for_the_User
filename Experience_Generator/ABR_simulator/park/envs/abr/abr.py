...
class ABREnv(core.SysEnv):
    ...
    * REFERENCE *
    ...
    def __init__(self):
        ...
        if not os.path.exists(park.__path__[0] + '/envs/abr/traces/'):
            wget.download(
                'https://www.dropbox.com/s/qw0tmgayh5d6714/cooked_traces.zip?dl=1',
                out=park.__path__[0] + '/envs/abr/')
            with zipfile.ZipFile(
                 park.__path__[0] + '/envs/abr/traces.zip', 'r') as zip_f:
                zip_f.extractall(park.__path__[0] + '/envs/abr/')
        ...
        self.all_traces = os.listdir(park.__path__[0] + '/envs/abr/traces/')
    ...
    def run(self, agent, *args, **kwargs):
        ...
        p = subprocess.Popen('mm-delay 40' +
            ...
            park.__path__[0] + '/envs/abr/traces/' + trace_file +
